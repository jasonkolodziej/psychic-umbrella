<script lang="ts">
	...
	let iconClass =
		'flex-shrink-0 w-6 h-6 text-primary-500 transition duration-75 group-hover:text-primary-900 dark:text-primary-400 dark:group-hover:text-white';
	let itemClass =
		'flex items-center p-2 text-base text-primary-900 transition duration-75 rounded-lg hover:bg-primary-100 group dark:text-primary-200 dark:hover:bg-primary-700';
...
<Sidebar
	...
	activeClass="bg-primary-100 dark:bg-primary-700"
	asideClass="fixed inset-0 z-30 flex-none h-full w-64 lg:h-auto border-e border-primary-200 dark:border-primary-600 lg:overflow-y-visible lg:pt-16 lg:block"
>
	<h4 class="sr-only">Main menu</h4>
	<SidebarWrapper
		divClass="overflow-y-auto px-3 pt-20 lg:pt-5 h-full bg-white scrolling-touch max-w-2xs lg:h-[calc(100vh-4rem)] lg:block dark:bg-primary-800 lg:me-0 lg:sticky top-2"
	>
		<nav class="divide-y divide-primary-200 dark:divide-primary-700">
...
<div
	hidden={drawerHidden}
	class="fixed inset-0 z-20 bg-primary-900/50 dark:bg-primary-900/60"
